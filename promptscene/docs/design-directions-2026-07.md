...
## D6. 상담층 — "견적 + 브리프" (2026-07 후속 결정)

> 2026-07(V1 다트 착수 시점) 추가. 배경: `/compose-room`이 자연어 한 줄을 받아
> 기능을 선택·조립하지만, "무엇을 지을 수 있고 / 무엇을 물어봐야 하는가"를
> 대화로 좁히는 층은 아직 없다. 이 절은 그 **상담층(consultation layer)**의
> 설계 방향을 못박는다. (구현 아님 — 방향 결정 기록.)

**결정:** compose-room 앞단에 **상담층 = 견적(estimate) + 브리프(brief)**를 둔다.
사용자의 요청을 받으면 (1) 지금 능력으로 **지어줄 수 있는 범위를 견적**하고,
(2) 남은 결정들을 **브리프 질문**으로 좁힌다.

**브리프 질문은 capability-map에서 파생한다 (핵심 규율):**
- **지킬 수 있는 것만 질문한다.** 카탈로그·검증된 기계로 실제 실현 가능한
  선택지만 브리프에 올린다. 실현 못 하는 걸 물어 기대를 만들지 않는다(정직 계약의
  대화판).
- **⛔(현재 불가) 항목은 질문하지 않는다.** 대신 사용자의 **의향을 기록**하고
  **"개척 청구서"(pioneering invoice)** 로 남긴다 — "이건 지금 못 하지만 원한다"는
  수요 신호를, 다음에 어느 SYSTEMS/기계를 해동할지의 근거로 축적한다.
- **기본값 제안 원칙 — 갈림길만 질문한다.** 합리적 기본값이 있는 결정은 묻지 말고
  제안하고 진행한다(도구의 AskUserQuestion 규율과 동형). 진짜 분기(mutually
  exclusive, 되돌리기 어려운 외형/토폴로지 선택)에서만 사용자에게 판정을 넘긴다.

**composition-plan → 룸 브리프로 진화.** 현재 `composition-plan.json`(계획 vs 실행
분리용 박제물)을 상담 결과를 담는 **룸 브리프**로 확장한다: 선택 기능 + 파라미터 +
`notSelected`(음성 증거) + **개척 청구서**(의향 기록) + 기본값 출처. 계획이 곧 상담
기록이 되어, "왜 이렇게 지었나"가 아티팩트로 남는다.

**외형 질문 3종은 D3 수요 발생 지점.** "무슨 소품 모양 / 어떤 색·재질 / 어떤 분위기"
같은 **외형(appearance) 질문**은 브리프가 다룰 수 있어야 완결되는데, 이는 [D3](#d3-생성-에셋-파이프라인--라이브러리-공장-모델)
생성 에셋 파이프라인(라이브러리 공장)이 있어야 대답 가능하다. 즉 **외형 3종 질문이
실제로 브리프에 등장하는 순간이 D3 착수의 수요 신호** — D2의 "서로 통신하는 기능 2개"가
COMPOSITIONS 착수 신호였던 것과 동형의 수요-구동 착수 규율.

**근거:** 상담층은 "자연어→씬"의 명세 부족 문제([D1](#d1-프로젝트의-정직한-사거리-재정의))를
대화로 흡수하되, capability-map에 묶어 **지킬 수 있는 약속만** 하게 만든다. 능력의
경계(✅/⛔)가 곧 질문의 경계가 되므로, 상담이 곧 정직 계약의 사용자 대면 표현이 된다.
