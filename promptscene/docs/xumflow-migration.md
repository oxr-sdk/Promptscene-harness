...
- **정식 MCP 셋업(후속) — 버전 확정: `com.ivanmurzak.unity.mcp@0.66.0`** (UXM 리포 검증문서로 직접 확인 — 아래 "MCP 버전 확정" 참조). 절차: 패키지 제거(완료) → `Assets/Plugins/NuGet/` 폴더(McpPlugin*/ReflectorNet + 공유 DLL) + `.nuget-installed.json` 마커 삭제 → `@0.66.0` 재설치(scopedRegistry 존치) → AI Game Dev Installer가 0.66.0 일치 DLL·서버바이너리 복원 → **UPM 버전 == NuGet DLL 버전(0.66.0) 확인** → 포트 27827 → 세션 재시작. (에디터 닫고 DLL 삭제 → 재오픈이 안전. 이 작업은 §5 기록·커밋 후.)
