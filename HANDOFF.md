...
>
> **최종 갱신:** 2026-08-13 (셸 3차 — HUD glass v6.4 전 게이트 PASS, §8-0zf).
>
...
| S2 | **studio 2클라 인프라**(ParrelSync 클론 + 역할 브리지) | ✅ **라이브 검증(2026-08-19)**: `AssembleRoom`에서 **빈 룸 2인 스폰 4신호** + **Chat 양방향 4신호** PASS = studio 최초 2인. 매니페스트 무수정(md5 불변). 절차 [build-studio-room.md](promptscene/docs/build-studio-room.md) §6.6, 트랩표·상세 [xumflow-migration.md](promptscene/docs/xumflow-migration.md) §17. **남은 파리티(Grab·과녁/점수) = 룸 부재로 미실행** |
...
- `/promptscene:cross-platform-ui <PC|PCSS|PCXR|Cross> [on <Room>]` — **studio판:** 레지스트리 순회 World Space uGUI HUD(토글형 FEATURE마다 버튼) + XR world-click(`XRWorldClicker`+`SubmitExternalRay`) + QuickTest 증명. add-component §6이 부품으로 호출. **디자인 = glass v6.4, U1~U12 전 게이트 PASS(2026-08-13, §8-0zf).** 토큰 SSOT = `assets/HudTheme.cs` 헤더(성역 — 변경은 플래그+세션 재시작), 절차·함정 = [build-studio-room.md](promptscene/docs/build-studio-room.md) **§5.1/§5.2**.
...
> ⚠️ **이 §5 체인은 2026-07-22에서 멈춰 있다.** 그 이후 세션(07-23 studio 관통부터 08-12 셸 1차까지)은 전부
> **§8 "바로 이어서"의 번호 항목**(`0x` / `0z` / `0za` / `0zc` / `0zd`)에 기록됐다. **최신 상태를 알려면 §8부터 읽어라.**
...
- ⛔ **studio 2클라(ParrelSync 클론) 함정 5종 — 상세·표는 [xumflow-migration.md](promptscene/docs/xumflow-migration.md) §17.5, 절차는 [build-studio-room.md](promptscene/docs/build-studio-room.md) §6.6.**
  ① **심링크 = 씬·설정·소스 공유** — A/B가 `QuickStart.unity`의 `startAsServer`를 같이 읽는다. 역할은 **런타임 분기**여야 하고, 클론 전용 상태 파일은 **프로젝트 루트**에 둔다(`Assets/` 안에 두면 원본까지 무장).
  ② ⛔ **클론 MCP의 NuGet 재복원이 B의 Play를 끊는다**(시작 몇 분 뒤 refresh→도메인 리로드). 조인 실패를 넷코드 탓하기 전에 B 로그에서 `[Unity-MCP DependencyResolver] Restoring`를 볼 것. 근본 대응 = **클론에서 MCP 비활성**.
  ③ ⛔ **클론 에디터 종료가 원본의 `unity-mcp-server`를 죽인다**(에디터는 멀쩡, 자동 재기동 없음). 복구 = `Library/mcp-server/win-x64/unity-mcp-server.exe port=21017 plugin-timeout=10000 client-transport=streamableHttp authorization=none`.
  ④ **MCP 포트는 고정이 아니라 `SHA256(프로젝트 경로)`** → 원본 21017 / 클론 25821. `UserSettings/`는 클론에 복사되지 않는다. **충돌 구조적 불가**.
  ⑤ **QuickTest 클라는 재시도가 없다** — B를 띄우기 전에 A가 host로 떠 있어야 한다. 그리고 `roomSceneKey`는 **씬을 저장해야** 반영된다(`playModeStartScene`이 디스크 에셋을 로드하므로 — 기존 "값이 되돌아간다" 증상의 정체).
...
**바로 이어서:**
0zg. ✅ **⭐ studio 2클라 인프라 확보 + studio 최초 2인 성립(2026-08-19, migration §17).** 지시 순서 ①→②→③ 고정 준수.
  **① 인프라 = ParrelSync 1.5.3, 매니페스트 무수정.** `.unitypackage`를 풀어 GUID 보존한 채 `Assets/ThirdParty/ParrelSync/`로 재배치(⛔ UPM git URL 금지) → Unity 6000.3.11f1 컴파일 0·메뉴 노출·`Packages/manifest.json` md5 **세션 전후 동일**. 클론 실측: `Assets`/`ProjectSettings`/`LocalPackages`=**심링크**, `Library`/`Packages`=**복사**, `UserSettings`=**없음**(→ MCP 포트가 갈리는 이유). 비용 ≈3 GB / 1분, 콜드 오픈 ≈5분.
  **역할 브리지(PromptScene 소유, shipped 무수정) = `Assets/PromptScene/Harness/Editor/`.** `startAsServer`의 소유자는 shipped `QuickTestStarter`(**`Assembly-CSharp`** — 씬의 `App.HotUpdate::` 표기는 stale 메타)이고 **값은 씬 에셋**에 산다 → 심링크로 공유되므로 런타임에 덮는 수밖에 없다. `[InitializeOnLoad]`가 `ClonesManager.IsClone()`을 읽어 `SessionState`에 기록 → `[RuntimeInitializeOnLoadMethod(AfterSceneLoad)]`가 private 필드를 리플렉션으로 뒤집는다. ⭐ **지시안의 "런타임 얇은 컴포넌트"를 없앴다** — `RuntimeInitializeOnLoadMethod`가 **Editor 어셈블리에서도 발화함을 실측 증명**했고, 그 덕에 하네스가 `App.HotUpdate`(Smart-Deploy 배포물)에 섞이지 않으며 **shipped 씬도 안 건드린다**. ⚠ fail-closed를 "안 걸렸다"가 아니라 **재현으로** 증명(원본 Play → 마커 `applied=False` = 훅은 **돌았고** 아무것도 안 함 + 값 유지 + 아바타 스폰 + 씬 md5 불변).
  **② 게이트 1 — 빈 룸 2인 스폰 ✅ 4신호 PASS(studio 최초 2인).** `AssembleRoom`, A=host/B=클론 자동 client: A 자기 아바타 `IsOwner=True` / B에 원격 `Desktop(Clone)` `IsOwner=False`(+자기 것 `owner=True`) / A 이동 `(5,0,3)` → B 관측 일치 / objId 집합 `{0..6}` 양측 동일·소유만 반전. **재조인**도 성립(새 clientId·새 아바타, 서버 소유 `ChatChannel` 생존, 고아 0).
  **③ 게이트 2 — 🟡 1/3 PASS.** **Chat 양방향 4신호 PASS**(A→B / B→A / 순서 보존 / 발신자 id `0,0,1,1` 교차 일치, 채널 objId 양측 동일). 보너스 실측 2건: **백필 없음**(재조인 B는 조인 후 1건만, A는 5건 — `BufferLast` 미사용 설계가 2클라에서 실제로 그렇게 동작) · **게스트 디스폰 정리**(고아 0).
...
0zf. ✅ **셸 3차 — 유리 방향 완성 + 대비 게이트 색공간 교정. `§5/§6 CROSS-PLATFORM-UI VERDICT: PASS`(전 게이트 통과, 세 세션 만에 처음 · 2026-08-12~13).**
  ...
  **⛔ 핵심 발견 — U7 대비 산술이 렌더러와 다른 색 공간에서 돌고 있었다.** 프로젝트는 Linear인데 게이트가 감마 공간에서 합성 → **반투명 스택 숫자가 전부 틀렸다**(어두운 잉크엔 비관적, 밝은 잉크엔 낙관적). 이 오차가 만든 결정 2건 = "Film .28→.60", "글리프 헤일로". **사람 결정 = 게이트를 Linear로 교정**(헤일로 값은 존치). 교정 후 예측 `#959595` vs 캡처 실측 `#949494` = 1/255 일치. 상세 §5.1.
  **⛔ 검증기 자체의 조용한 실패 4건 발견·수정**(백지 캡처 / 실행마다 달라진 캡처 배율 / 옛 액센트 기대값 / **Teardown이 복원 실패를 Log로만 남김**) — 표와 재현 절차는 **§5.2**. 마지막 건은 차단 재현으로 검증(Temp 사본 삭제 후 복원 성공).
  **구조 개선 2건:** ① 잉크를 **역할 토큰**(`GlyphInk`/`GlyphHalo`, 규약=항상 반대 극)으로 승격 — 코드 5곳에 색이 박혀 있던 게 이 세션 게이트 드리프트 2건의 원인이었다 ② 헤일로 두께를 **역할별**(`OutlineW` 2px / `LabelHaloW` 1px)로 분리 — 뭉개짐은 절대 px이 아니라 상대 두께가 정한다(1/8 버림 · 1/24·1/16 통과). ③ 아이콘 폰트 굽는 조건이 어디에도 없어 면적 대조로 역산 → **`assets/bake_icon_font.py`** 신설(wght 400·GRAD 0·**opsz 48**·FILL).
  **최종:** U1~U12 전부 PASS(대비 위반 0 · 리터럴 0 · 보장 없는 잉크 0 · 액센트 PROVEN · 아이콘 8/8), WARN 1(폰트 폴백=baked-base), **Error 0**, QuickStart 원상복구. 성역 플래그 **제거 완료**(다음 세션부터 가드 복구).
  **남은 것(전부 취향 판단, 결함 아님):** ① 투명도 .18·미러 세기 ② **KeyBadge만 극이 반대**(어두운 키캡+밝은 헤일로 — 자기 Scrim을 든 별개 맥락, 최악 4.31:1로 성립) ③ 밝은 환경 라벨 무게(더 가볍게 하려면 라벨 제거뿐).
0ze. ✅ **셸 2차 — U7 구조 규칙 재정의 + KeyBadge 헤일로(2026-08-12). 토큰 편집은 플래그 대기였고 → 0zf에서 전부 해소.**
  **⭐ U7 구조 규칙을 판에서 잉크로 옮겼다(사람 결정 = "게이트를 갱신").** 구 규칙 `"Scrim 없는 Film = FAIL"`은 유리 방향(Scrim α0)에서 디자인 전체를 구조 FAIL로 만드는데, 정작 막으려던 위험은 **불투명 헤일로**가 이미 다른 기계로 막는다. 신 규칙 = **"Scrim도 헤일로도 없이 Film 위에 놓인 잉크가 있으면 FAIL"**(헤일로 조건은 halo 절과 **같은 단정**을 재사용 = 완화 아님). 잉크 없는 Film은 통과시키되 개수를 따로 보고. **"덮인다"는 계층이 아니라 기하(`Covers`)로 판정** — 원판(`__disc`)과 글리프(`__glyph`)는 형제라 계층으로는 서로 안 보인다.
  **KeyBadge 키캡 헤일로:** 죽은 `Outline()`(=`TextOutline`, 감싸는 잉크와 **같은 #0A0D12**라 자기대비 1.00:1인 함정)을 `Halo()`로 교체. ⚠ KeyBadge는 지금도 **어두운 키캡 + 밝은 헤일로**로 HUD와 극이 반대다(자기 Scrim을 든 별개 맥락 — §8-0zf "남은 것" 참고).
  **⚠ 사람 결정(여전히 유효) — Scrim·흰 테두리 제거는 `AssembleRoom.unity` 씬에만 유지하고 `assemble_ui.cs` 기본값으로 **승격하지 않는다.** 따라서 `/cross-platform-ui`를 재실행하면 Scrim·테두리가 **되살아난다 — 알려진 동작이지 결함이 아니다.**
0zd. 🟡 **셸(소환 런치패드) 1차 — F1 소환 + 정면 배치 + 미감 반복; ⛔ 성역 플래그에서 멈춤(2026-08-11~12).**
  **선행 정찰 2건이 먼저 있었다.** ① `ui-surface-survey.md` — 셸 action-space 실측 인벤토리(구현 0건). ② 그 문서 **§12 라이브 프로브**(Play 1회, 읽기 전용): **EventSystem 승자 = `[QuickTest] EventSystem`(StandaloneInputModule), Play 2회 재현 동일 = 결정적**이고 패자(아바타 `InputSystemUIInputModule`)는 `currentInputModule=NULL`이라 선택을 못 가진다. **⛔ 정적 grep이 뒤집힌 지점: `M` 키는 비어 있지 않다**(`XR Interaction Controller Controls › Menu` + `Hand Controls › Pinch`). 라이브 `ListEnabledActions()`는 활성 액션 152개 / `<Keyboard>` 바인딩 85건 / 서로 다른 키 51개를 보여줬고, grep이 못 본 출처가 셋 — Controller/Hand Controls(시뮬레이터 장치 에뮬), 그리고 **`(no asset) › Debug Menu`(코드 생성 액션맵 = 원리적으로 grep 불가)**. `G I J K L N O P`·`1~8`도 전부 점유. **두 맥락 모두에서 검증된 빈 키는 `F1`뿐**(`F2~F12`도 free). 보이스챗은 **실재**(MetaVc `isInputMuted`/`isDeafened` 쓰기·원복 예외 0, 마이크 장치 실물) — 단 가청/2클라는 MPPM 부재로 **절반만 증명**.
  **구현(전부 studio, 사람 확인 완료):** `HudSummon.cs`(**F1** 데스크톱 / XR 왼손 `secondaryButton`; **GameObject가 아니라 Canvas+raycaster를 토글** — GO를 끄면 자기 자신이 꺼져 되살릴 수 없다) · `HudPlacement.cs`(**`FrontOfFace`** 기본: 수평 시선 방향 `reachDistance` 0.55 m, 눈높이 −0.15; `LeftHandSide` 모드 보존. **회전은 안 건드린다** — `CrossPlatformRoomHud.LateUpdate`가 빌보드를 이미 소유하므로 위치·스케일만 씀. 아바타 밑으로 **재부모 안 함**(디스폰 동반 방지). `preserveApparentSize`로 거리/1.5 배율 → **각크기 20.4° 유지 = U6 가독성 판정 승계**). ⚠️ 왼손 앵커가 처음 안 보였던 실측 이유: 카메라 로컬 `z = −0.059`(**카메라 평면 뒤**).
  **`TextInputFocus.cs`(SYSTEMS 신규, IRoomCore 무변경):** `SuppressWorldClick` 동형 클레임 API. **⛔ EventSystem을 쿼리하면 fail-open** — 데스크톱 채팅은 IMGUI라 포커스가 `GUI.GetNameOfFocusedControl()`에 있고 EventSystem에 안 보인다(VR 입력창만 막히고 데스크톱은 뚫림). 소비 3곳(ChatContent/ChatWorldPanel 클레임, ChairSitContent 확인). **U13 게이트 PASS** — 핵심은 **E 키 차등**(억제 OFF→`SitKeyPressed=True` / ON→`False`, 같은 키 주입) + 누수 프루닝(fail-**closed** 방지) + 세션 간 리셋. **⛔ WASD는 범위 밖** — `DummyController`가 PromptScene 폴더 밖(studio 앱 코드)이라 소유권 미승인 → 채팅에 `wasd` 치면 **여전히 걸어간다**(청구서 I-7b, 조사서 미기입).
  **미감 반복(사람 지시):** 검정 Scrim·흰 테두리 제거(**알파 0 — 컴포넌트를 끄지 않는다**: `raycastTarget`이 살아야 여백 클릭이 월드로 안 샌다) · 글리프 **밝은 헤일로**(`TextHi` 2px). ⛔ **`TextOutline`을 쓰면 안 된다 — `GlyphDark`와 같은 잉크(#0A0D12)라 자기대비 1.00:1, 보강 0** · **액센트를 채움→테두리로 이동**(ON = ring `Accent`, disc는 유리 유지 → 아이콘이 파랗게 안 물듦; `Roles.AccentBearing`이 Disc·Ring 둘 다 규정해 역할 규약 위반 아님).
  **⛔ 여기서 멈췄다 — 남은 미감이 전부 성역 `HudTheme.cs` 안에 있다**(`Film` 투명도 + `HudSprites.RingGraded` 글라스 테두리). 훅 탈출구는 `PROMPTSCENE_ALLOW_THEME_EDIT=1`인데 **에이전트가 켤 수 없다**(훅은 시작 시 환경을 물려받음 = 의도된 마찰). 대신 쓰려다 **자동 권한 분류기에도 차단**됨 — 우회하지 않았다. **사람이 `.claude/settings.local.json`(git 미추적 — `settings.json`은 커밋돼 가드가 팀 전체에 영구히 풀림)에 `env` 추가 → 세션 재시작.**
  **재개 시 할 일:** `Film` 0.60→0.30 · `FilmHover` 0.70→0.40 · `RingGraded` 반사 하이라이트 → **Phase 1b로 studio 사본 덮어쓰기** → 밝은/어두운 배경 캡처로 헤일로 육안 확인(이 프로젝트에서 **산술 통과를 눈이 두 번 잡았다**) → 플래그 제거. **미결 2건 → ✅ 둘 다 §8-0ze에서 사람 결정 받음**(① AssembleRoom에만 유지·기본값 승격 안 함 ② U7 게이트 갱신 — 구조 규칙 재정의 완료, 라벨 1.65:1만 잔존). SSOT [ui-surface-survey.md](promptscene/docs/ui-surface-survey.md) **§12**.
