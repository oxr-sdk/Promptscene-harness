...
0zd. 🟡 **셸(소환 런치패드) 1차 — F1 소환 + 정면 배치 + 미감 반복; ⛔ 성역 플래그에서 멈춤(2026-08-11~12).**
  ...
  **재개 시 할 일:** `Film` 0.60→0.30 · `FilmHover` 0.70→0.40 · `RingGraded` 반사 하이라이트 → **Phase 1b로 studio 사본 덮어쓰기** → 밝은/어두운 배경 캡처로 헤일로 육안 확인(이 프로젝트에서 **산술 통과를 눈이 두 번 잡았다**) → 플래그 제거. **미결 2건:** ① Scrim·테두리 제거는 **`AssembleRoom.unity` 씬에만** — `/cross-platform-ui` 재실행 시 `assemble_ui.cs`가 되살린다(모든 룸 기본값 변경이라 승인 대기) ② **U7 대비 게이트는 지금 구성에서 라벨 1.65:1로 FAIL이 정상**(Scrim 제거의 의도된 결과) — 게이트 처리 방침 미정. SSOT [ui-surface-survey.md](promptscene/docs/ui-surface-survey.md) **§12**.
