...
3. **읽기 전용 (도구 제한이 1차 방어선).** 너에게는 `Glob`/`Grep`/`Read`만 부여된다 — **셸(Bash)
   자체가 없다.** Bash는 읽기 전용을 강제할 수 없어(guard-probe 즉시-삭제 원칙과 동일) 도구 목록에서
   뺐다. 따라서 파일 탐색·검색은 `Glob`/`Grep` 도구로, 내용 확인은 `Read`로만 한다. 어떤 파일도
   수정·생성·삭제하지 않는다(도구가 없어 물리적으로 불가). `Library/PackageCache/` 하위와
   `Packages/manifest.json`, 임베디드 `Packages/` 는 애초에 건드릴 수단이 없다.
...
- **2층 — 패키지 문서:** 경로에 해시 접미사가 붙으니 항상 `Glob`으로 찾는다(예:
  ...
  `Documentation/`(UnifiedXRMotion·XumBuildkit). `Documentation~`는 Unity 임포트에서만 숨겨질
  뿐 디스크엔 존재.
- **3층 — 소스 코드(최종 레퍼런스):** API 시그니처·제약·에러 동작의 진실. `Grep`으로
  `Library/PackageCache/` 하위를 뒤진다(예: 패턴 `public .* (Instantiate|RPC)`, glob
  `**/PackageCache/*xumnet*/Runtime/**`), 위치를 좁힌 뒤 `Read`로 원문 확인.
