...
description: >
  ...
  wants to build or deploy the client for a headset/device, e.g. "/build-client Meta",
...
master-connect → room-join → move). Mechanics live in **`c:\J_0\docs\build-meta-client.md`** — read it;
...
## Phase 1 — Apply preset + persist defines  (`assets/build_client.cs`, set `PLATFORM`)
...
## Phase 2 — Configure + build  (same `assets/build_client.cs`, second pass)
...
## Phase 3 — Deploy (Android targets)  (`assets/deploy_android.ps1`)
...
3. In `assets/build_client.cs`, add the `case "<Platform>"` mapping (preset, presetDir, target, isXR, isFlat).
